function drawFaceModel(landmarks) {
	function draw(points) {
		faceModel.moveTo(Math.round(landmarks[points[0]][0] * 1.5), Math.round(landmarks[points[0]][1] * 1.5));

		for (let i = 1; i < points.length; i += 1) {
			faceModel.lineTo(Math.round(landmarks[points[i]][0] * 1.5), Math.round(landmarks[points[i]][1] * 1.5));
		}
	}
...
function drawBackground(c1, c2, steps) {
	...
	for (let i = 0; i < steps; i += 1) {
		...
		backgroundBMD.rect(0, y, K_PROJECT_WIDTH, y + yStep, Phaser.Color.getWebRGB(c));
...
const playState = {
	create: function() {
		...
		faceModel.position = {x: -(K_PROJECT_WIDTH / 4), y: -(K_PROJECT_HEIGHT / 2)};
		faceModel.visible = true;
		...
		userEmotionalStateText = game.add.text(0, 0, "neutral", {fontSize: "80px", fill: "#80ff00"});
		...
		attractText = game.add.text(0, 0, "meet the new AI that knows you\nbetter than you know yourself", {fontSize: "80px", fill: "#80ff00"});
		attractText.anchor.setTo(0.5, 0.5);
		attractText.position = {x: K_PROJECT_WIDTH / 2, y: K_PROJECT_HEIGHT / 2};
		attractText.visible = false;


		eventText = game.add.text(0, 0, "                     ", {font: "Arial, Helvetica, sans-serif", fontSize: "120px", fontWeight: "bold", fill: "#000000", backgroundColor: "#ffff1a"});
		eventText.anchor.setTo(0.5, 0.5);
		eventText.visible = false;

		eventScoreText = game.add.text(0, 0, "                    ", {font: "Arial, Helvetica, sans-serif", fontSize: "90px", fontWeight: "bold", fill: "#33cc33", backgroundColor: "#000000"});
		eventScoreText.anchor.setTo(0.5, 0.5);
		eventScoreText.visible = false;

		positivePercentText = game.add.text(0, 0, "                    ", {font: "Arial, Helvetica, sans-serif", fontSize: "90px", fontWeight: "bold", fill: "#33cc33", backgroundColor: "#000000"});
		positivePercentText.anchor.setTo(0.5, 0.5);
		positivePercentText.visible = false;

		negativePercentText = game.add.text(0, 0, "                    ", {font: "Arial, Helvetica, sans-serif", fontSize: "90px", fontWeight: "bold", fill: "#ff0000", backgroundColor: "#000000"});
		negativePercentText.anchor.setTo(0.5, 0.5);
		negativePercentText.visible = false;

		estimatedIncomeText = game.add.text(0, 0, "                      ", {font: "Arial, Helvetica, sans-serif", fontSize: "100px", fontWeight: "bold", fill: "#000000", backgroundColor: "#ffff1a"});
		estimatedIncomeText.anchor.setTo(0.5, 0.5);
		estimatedIncomeText.visible = false;

		mentalHealthText = game.add.text(0, 0, "                      ", {font: "Arial, Helvetica, sans-serif", fontSize: "100px", fontWeight: "bold", fill: "#000000", backgroundColor: "#ffff1a"});
		mentalHealthText.anchor.setTo(0.5, 0.5);
		mentalHealthText.visible = false;

		selfImageText = game.add.text(0, 0, "                      ", {font: "Arial, Helvetica, sans-serif", fontSize: "100px", fontWeight: "bold", fill: "#000000", backgroundColor: "#ffff1a"});
		selfImageText.anchor.setTo(0.5, 0.5);
		selfImageText.visible = false;

		iqText = game.add.text(0, 0, "     ", {font: "Arial, Helvetica, sans-serif", fontSize: "300px", fontWeight: "bold", fill: "#ffff1a", backgroundColor: "#000000"});
	...
	update: function() {
		...
		if (data) {
			...
			userEmotionalStateText.position = {x: (data.landmarks[14][0] * 1.5 * 2) - 200, y: (data.landmarks[14][1] * 1.5 * 2) - 600};
		} else {
			if (faceModelFadeDebounceHandle === null) {
				faceModelFadeDebounceHandle = setTimeout(() => {
					...
					if (!data) {
						const fadeModel = game.add.tween(faceModelGroup).to({alpha: 0.0}, 500, Phaser.Easing.Linear.None, true, 0, 0, false);

						fadeModel.onComplete.addOnce(() => {
							attractText.alpha = 0.0;
							attractText.visible = true;

							attractTween = game.add.tween(attractText).to({alpha: 1.0}, 1000, Phaser.Easing.Linear.None, true, 0, -1, true);
		...
		if (dogsEvent) {
			...
			eventScoreText.text = `${data[1]} ${data[0] ? "positive" : "negative"} emotion`;
			...
			const eventTextTween = game.add.tween(eventText.position).to( {x: K_PROJECT_WIDTH / 2}, 300, Phaser.Easing.Linear.None, true, 0, 0, false);
			const eventScoreTextTween = game.add.tween(eventScoreText.position).to( {x: K_PROJECT_WIDTH / 2}, 300, Phaser.Easing.Linear.None, true, 0, 0, false);

			eventTextTween.onComplete.addOnce(() => {
				const eventTextTween = game.add.tween(eventText.position).to( {x: -(eventText.width)}, 300, Phaser.Easing.Linear.None, true, 4000, 0, false);
				const eventScoreTextTween = game.add.tween(eventScoreText.position).to( {x: K_PROJECT_WIDTH + eventScoreText.width}, 300, Phaser.Easing.Linear.None, true, 4000, 0, false);
			});
		}
		...
		if (genderEvent) {
			...
			eventText.text = `User prefers ${data[0] ? "men" : "women"} vs. ${data[0] ? "women" : "men"}`;
			...
			eventTextTween.onComplete.addOnce(() => {
				const eventTextTween = game.add.tween(eventText.position).to( {x: K_PROJECT_WIDTH + eventText.width}, 300, Phaser.Easing.Linear.None, true, 4000, 0, false);
				const eventScoreTextTween = game.add.tween(eventScoreText.position).to( {x: -(eventScoreText.width)}, 300, Phaser.Easing.Linear.None, true, 4000, 0, false);
			});
		}
		...
		if (kanyeEndEvent) {
			...
			const eventTextTween = game.add.tween(eventText.position).to( {x: -(eventText.width)}, 200, Phaser.Easing.Linear.None, true, 0, 0, false);
			const eventScoreTextTween = game.add.tween(eventScoreText.position).to( {x: K_PROJECT_WIDTH + eventScoreText.width}, 200, Phaser.Easing.Linear.None, true, 0, 0, false);
		}
		...
		if (pizzaEndEvent) {
			...
			const eventTextTween = game.add.tween(eventText.position).to( {x: K_PROJECT_WIDTH + eventText.width}, 300, Phaser.Easing.Linear.None, true, 0, 0, false);
			const eventScoreTextTween = game.add.tween(eventScoreText.position).to( {x: -(eventScoreText.width)}, 300, Phaser.Easing.Linear.None, true, 0, 0, false);
		}
		...
		if (datingPreferenceEvent) {
			...
			negativePercentText.position.x = K_PROJECT_WIDTH + negativePercentText.width;
			negativePercentText.position.y = K_PROJECT_HEIGHT - 300;
			negativePercentText.visible = true;

			const eventTextTween = game.add.tween(eventText.position).to( {x: K_PROJECT_WIDTH / 2}, 200, Phaser.Easing.Linear.None, true, 0, 0, false);

			const positiveTextTween = game.add.tween(positivePercentText.position).to( {x: K_PROJECT_WIDTH / 4}, 200, Phaser.Easing.Linear.None, true, 0, 0, false);
			const negativeTextTween = game.add.tween(negativePercentText.position).to( {x: K_PROJECT_WIDTH * 0.75}, 200, Phaser.Easing.Linear.None, true, 0, 0, false);

			eventTextTween.onComplete.addOnce(() => {
				const eventTextTween = game.add.tween(eventText.position).to( {x: K_PROJECT_WIDTH + eventText.width}, 200, Phaser.Easing.Linear.None, true, 3000, 0, false);
				const positiveTextTween = game.add.tween(positivePercentText.position).to( {x: -(positivePercentText.width)}, 200, Phaser.Easing.Linear.None, true, 3000, 0, false);
				const negativeTextTween = game.add.tween(negativePercentText.position).to( {x: K_PROJECT_WIDTH + negativePercentText.width}, 200, Phaser.Easing.Linear.None, true, 3000, 0, false);
			});
		}
		...
		if (socialNewsEvent) {
			...
			eventTextTween.onComplete.addOnce(() => {
				const eventTextTween = game.add.tween(eventText.position).to( {x: -(eventText.width)}, 200, Phaser.Easing.Linear.None, true, 3000, 0, false);
				const positiveTextTween = game.add.tween(positivePercentText.position).to( {x: -(positivePercentText.width)}, 200, Phaser.Easing.Linear.None, true, 3000, 0, false);
				const negativeTextTween = game.add.tween(negativePercentText.position).to( {x: K_PROJECT_WIDTH + negativePercentText.width}, 200, Phaser.Easing.Linear.None, true, 3000, 0, false);
			});
		}
		...
		if (iqEvent) {
			...
			const eventTextTween = game.add.tween(eventText.position).to( {x: K_PROJECT_WIDTH / 2}, 300, Phaser.Easing.Linear.None, true, 0, 0, false);
			const iqTextTween = game.add.tween(iqText.position).to( {x: K_PROJECT_WIDTH / 2}, 300, Phaser.Easing.Linear.None, true, 0, 0, false);

			eventTextTween.onComplete.addOnce(() => {
				const eventTextTween = game.add.tween(eventText.position).to( {x: -(eventText.width)}, 300, Phaser.Easing.Linear.None, true, 4000, 0, false);
				const iqTextTween = game.add.tween(iqText.position).to( {x: K_PROJECT_WIDTH + iqText.width}, 300, Phaser.Easing.Linear.None, true, 4000, 0, false);
			});
		}
		...
		if (mentalHealthEvent) {
			...
			mentalHealthText.text = `possible mental illness: ${data[0] ? "yes" : "no"}`;
