...
function makeAd(x, y, headline, url, body) {
	...
	const headlineText = game.add.text(x + 7, y + 7, headline, {font: "Arimo, sans-serif", fontSize: "30px", fontWeight: "normal", fill: "#2200CC"});
	headlineText.wordWrap = true;
	headlineText.wordWrapWidth = 450;
	adGroup.add(headlineText);

	const linkText = game.add.text(x + 7, headlineText.y + headlineText.height, url, {font: "Arimo, sans-serif", fontSize: "19px", fontWeight: "normal", fill: "#0F9D58"});
	linkText.wordWrap = true;
	linkText.wordWrapWidth = 450;
	adGroup.add(linkText);

	const bodyText = game.add.text(x + 7, linkText.y + linkText.height, body, {font: "Arimo, sans-serif", fontSize: "19px", fontWeight: "normal", fill: "#000000"});
...
function makeTextBox(x, y, copy, invert = false) {
	...
	textBoxGroup.copyText = game.add.text(x + 12, y + 12, copy, {font: "Arimo, sans-serif", fontSize: "45px", fontWeight: "bold", fill: textcolor});
...
function drawDogFilter(landmarks) {
	if (landmarks) {
		if (frame % K_AR_FRAME_INTERVAL === 0) {
			...
			dogNose.position.x = landmarks[30][0] + userVideoGroup.position.x + userVideoSprite.position.x;
			dogNose.position.y = landmarks[30][1] + userVideoGroup.position.y + userVideoSprite.position.y;
			...
			perpendicular = perpendicular.fromAngle(median.x, median.y, cline.angle - 1.5708, cd * 0.4);
...
function drawTophatFilter(landmarks) {
	if (landmarks) {
		if (frame % K_AR_FRAME_INTERVAL === 0) {
			...
			const median = cline.midPoint();
			let perpendicular = new Phaser.Line(median.x, median.y, 0, 0);
			perpendicular = perpendicular.fromAngle(median.x, median.y, cline.angle - 1.5708, cd * 0.2);
			...
			monocle.position = {x: rightPupil[0] + userVideoGroup.position.x + userVideoSprite.position.x, y: rightPupil[1] + userVideoGroup.position.y + userVideoSprite.position.y};
			monocle.scale.setTo(monocleScale, monocleScale);
			monocle.rotation = cline.angle;
			monocle.visible = true;

			mustache.position = {x: landmarks[33][0] + userVideoGroup.position.x + userVideoSprite.position.x, y: landmarks[33][1] + userVideoGroup.position.y + userVideoSprite.position.y};
...
function drawPoorHatFilter(landmarks) {
	if (landmarks) {
		if (frame % K_AR_FRAME_INTERVAL === 0) {
			...
			const median = cline.midPoint();
			let perpendicular = new Phaser.Line(median.x, median.y, 0, 0);
			perpendicular = perpendicular.fromAngle(median.x, median.y, cline.angle - 1.5708, cd * 0.1);
...
function drawHeartEyesFilter(landmarks) {
	if (landmarks) {
		if (frame % K_AR_FRAME_INTERVAL === 0) {
			...
			leftHeart.position = {x: leftPupil[0] + userVideoGroup.position.x + userVideoSprite.position.x, y: leftPupil[1] + userVideoGroup.position.y + userVideoSprite.position.y};
			leftHeart.scale.setTo(heartScale, heartScale);
			leftHeart.rotation = cline.angle;
			leftHeart.visible = true;

			rightHeart.position = {x: rightPupil[0] + userVideoGroup.position.x + userVideoSprite.position.x, y: rightPupil[1] + userVideoGroup.position.y + userVideoSprite.position.y};
...
function drawProhibitedFilter(landmarks) {
	if (landmarks) {
		if (frame % K_AR_FRAME_INTERVAL === 0) {
			...
			leftProhibited.position = {x: leftPupil[0] + userVideoGroup.position.x + userVideoSprite.position.x, y: leftPupil[1] + userVideoGroup.position.y + userVideoSprite.position.y};
			leftProhibited.scale.setTo(pScale, pScale);
			leftProhibited.rotation = cline.angle;
			leftProhibited.visible = true;

			rightProhibited.position = {x: rightPupil[0] + userVideoGroup.position.x + userVideoSprite.position.x, y: rightPupil[1] + userVideoGroup.position.y + userVideoSprite.position.y};
...
function drawThoughtBubbleFilter(landmarks, ref) {
	if (landmarks) {
		if (frame % K_AR_FRAME_INTERVAL === 0) {
			...
			ref.position = {x: landmarks[17][0] + userVideoGroup.position.x + userVideoSprite.position.x, y: landmarks[17][1] + userVideoGroup.position.y  + userVideoSprite.position.y};
...
makeMultiFrameEvent(61, 80, () => {
	...
	if (landmarks) {
		threeTitle.x = K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2);
...
makeMultiFrameEvent(80, 98, () => {
	...
	if (landmarks) {
		twoTitle.x = K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2);
...
makeMultiFrameEvent(98, 121, () => {
	...
	if (landmarks) {
		oneTitle.x = K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2);
...
makeMultiFrameEvent(121, 157, () => {
	...
	if (landmarks) {
		techQuizTitle.x = K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2);
...
makeMultiFrameEvent(297, 486, () => {
	...
	if (landmarks) {
		zuckAlertBoxPosition = {x: K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2) - 337, y: landmarks[33][1] - 140};
	}

	zuckScreenSpaceBMD.draw(drawKey, zuckAlertBoxPosition.x, zuckAlertBoxPosition.y);
...
makeMultiFrameEvent(486, 585, () => {
	...
	if (landmarks) {
		const x = K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2) + game.rnd.integerInRange(-175, 175);
		const y = landmarks[33][1] + game.rnd.integerInRange(-175, 175);

		const colorObject = Phaser.Color.getRGB(Phaser.Color.getRandomColor());
		const color = Phaser.Color.RGBtoString(colorObject.r, colorObject.g, colorObject.b, colorObject.a, "#");

		for (let i = 0; i < 10; i += 1) {
			const xo = game.rnd.integerInRange(-(Math.pow(5, i)), Math.pow(5, i));
			const yo = game.rnd.integerInRange(-(Math.pow(5, i)), Math.pow(5, i));
			questionMarks.push(game.add.text(x + xo, y + yo, "?", {font: "Arimo, sans-serif", fontSize: "150px", fontWeight: "bold", fill: color}));
		}
	}
});
...
makeMultiFrameEvent(606, 607, () => {
	...
	if (dogPos !== null && dogNeg !== null) {
		...
		dogStats.text = likeDogs ? `${(dogPos - dogNeg).toFixed(6)} \npositive ` : `${(dogNeg - dogPos).toFixed(6)} \nnegative `;
		...
		if (K_INSTALLATION_MODE) {
			socket.emit("dogs", [likeDogs, likeDogs ? (dogPos - dogNeg).toFixed(6) : (dogNeg - dogPos).toFixed(6)]);
		}
	}

	if (likeDogs) {
		dogEmojiEmitter.makeParticles("smileEmoji", 0, K_EMOJI_EXPLOSION_PARTICLES, true, true);
		youLikeDogsSFX.play();
	} else {
		dogEmojiEmitter.makeParticles("frownEmoji", 0, K_EMOJI_EXPLOSION_PARTICLES, true, true);
		youDontLikeDogsSFX.play();
	}

	dogTextBox = makeTextBox(K_PROJECT_WIDTH / 2 - (K_TEXT_BOX_SIZE / 2), K_PROJECT_HEIGHT / 2 - (K_TEXT_BOX_SIZE / 2), "Your reaction to dog content");
...
makeMultiFrameEvent(1440, 1441, () => {
	...
	if (menPos !== null && womenPos !== null) {
		...
		genderStatsBias.text = preferMen ? `+${(menPos - womenPos).toFixed(6)} \n male bias ` : `+${(womenPos - menPos).toFixed(6)} \n female bias `;

		// We only emit a visualization event if we got some signal from the user
		if (K_INSTALLATION_MODE) {
			socket.emit("gender", [preferMen, preferMen ? (menPos - womenPos).toFixed(6) : (womenPos - menPos).toFixed(6)]);
		}
	}
...
makeMultiFrameEvent(1713, 1714, () => {
	...
	const kanyeTitleSlideTween = game.add.tween(kanyeTitle.position).to( {x: K_PROJECT_WIDTH / 2}, 300, Phaser.Easing.Linear.None, true, 0, 0, false);
	const kanyeBiasSlideTween = game.add.tween(kanyeBias.position).to( {x: K_PROJECT_WIDTH / 2}, 300, Phaser.Easing.Linear.None, true, 0, 0, false);
...
makeMultiFrameEvent(1797, 1798, () => {
	const kanyeTitleSlideTween = game.add.tween(kanyeTitle.position).to( {x: -(kanyeTitle.width / 2)}, 300, Phaser.Easing.Linear.None, true, 0, 0, false);
	const kanyeBiasSlideTween = game.add.tween(kanyeBias.position).to( {x: K_PROJECT_WIDTH + (kanyeTitle.width / 2)}, 300, Phaser.Easing.Linear.None, true, 0, 0, false);
...
makeMultiFrameEvent(1808, 1809, () => {
	...
	const pizzaTitleSlideTween = game.add.tween(pizzaTitle.position).to( {x: K_PROJECT_WIDTH / 2}, 300, Phaser.Easing.Linear.None, true, 0, 0, false);
	const pizzaBiasSlideTween = game.add.tween(pizzaBias.position).to( {x: K_PROJECT_WIDTH / 2}, 300, Phaser.Easing.Linear.None, true, 0, 0, false);
...
makeMultiFrameEvent(1893, 1894, () => {
	...
	if (likeKanye) {
		...
		body = "Since you love Kanye, we're ONLY giving you access to FAVORABLE Kanye content! Click here for takes that align with your worldview!";
	} else {
		headline = "HATE KANYE?";
		url = "https://kanye-haters-only.com";
		body = "Since you hate Kanye, we're ONLY giving you access to UNFAVORABLE Kanye content! Click here for takes that align with your worldview!";
	}
...
makeMultiFrameEvent(1909, 1910, () => {
	...
	if (likePizza) {
		...
		body = "It's important that you ONLY see content for PIZZA HATERS such as yourself.";
	}
...
makeMultiFrameEvent(1928, 1929, () => {
	...
	if (likePizza) {
		...
		body = "Pizza is disgusting, and you're one of the people who understands that. Click here to enter the pizza-negative ideaology bubble.";
	}
...
makeMultiFrameEvent(1938, 1939, () => {
	...
	if (likeKanye) {
		...
		body = "The rules are simple: You have a positive perspective on Kanye. We bias your internet experience to feature more Kanye.";
	} else {
		headline = "KANYE IS NOT GOOD";
		url = "https://kanye-is-not-good.org";
		body = "You personally do not care for Kanye. We honor that by only showing you content that does not challenge your ideas.";
	}
...
makeMultiFrameEvent(1948, 1949, () => {
	...
	if (likePizza) {
		...
		body = "We have something in common: You know you like pizza, and WE know you like pizza. That's why you're seeing this ad.";
	} else {
		headline = "PIZZA HATER DETECTED";
		url = "https://no-to-pizza.com";
		body = "We have something in common: You know you dislike pizza, and WE know you dislike pizza. That's why you're seeing this ad.";
	}
...
makeMultiFrameEvent(1971, 1972, () => {
	...
	if (landmarks) {
		standbyForAIVisionTitle.x = K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2);
...
makeMultiFrameEvent(2125, 2278, () => {
	...
	if (gameframe % K_VIDEO_PROCESSING_FRAME_INTERVAL === 0 || frame === 2125) {
		const s = 1 / K_VIDEO_PROCESSING_DOWNRES_FACTOR * 1.50;
		userVideoProcessedBMD.copy(userVideoSprite, 0, 0, Math.floor(userVideo.width), Math.floor(userVideo.height), 0, 0, undefined, undefined, 0, 0, 0, s, s, 1, null, true);
	...
	if (landmarks) {
		...
		const xPos = landmarks[33][0] + userVideoGroup.position.x + userVideoSprite.position.x - (size / 2);
		const yPos = landmarks[33][1] + userVideoGroup.position.y + userVideoSprite.position.y - (size / 2);
		boundingBox.position = {x: xPos, y: yPos};
		boundingBox.drawRect(0, 0, size, size);
		lastKnownFaceBounding = {
			x: (xPos - userVideoGroup.position.x - userVideoSprite.position.x) / K_VIDEO_PROCESSING_DOWNRES_FACTOR, 
			y: (yPos - userVideoGroup.position.y - userVideoSprite.position.y) / K_VIDEO_PROCESSING_DOWNRES_FACTOR, 
...
makeMultiFrameEvent(2278, 2279, () => {
	...
	if (lastKnownFaceBounding.x !== null && lastKnownFaceBounding.x >= 0 && lastKnownFaceBounding.x + lastKnownFaceBounding.size < (userVideoSprite.width / K_VIDEO_PROCESSING_DOWNRES_FACTOR) - 1
		&& lastKnownFaceBounding.y !== null && lastKnownFaceBounding.y >= 0 && lastKnownFaceBounding.y + lastKnownFaceBounding.size < (userVideoSprite.height / K_VIDEO_PROCESSING_DOWNRES_FACTOR) - 1
			&& lastKnownFaceBounding.size !== null) {
				...
				userPhotoBMD = game.add.bitmapData(Math.round(lastKnownFaceBounding.size), Math.round(lastKnownFaceBounding.size));
	} else {
		top = (userVideoProcessedBMD.height / 2) - userVideoProcessedBMD.height / 4;
		bottom = (userVideoProcessedBMD.height / 2) + userVideoProcessedBMD.height / 4;
		...
		userPhotoBMD = game.add.bitmapData(Math.round(userVideoProcessedBMD.width / 2), Math.round(userVideoProcessedBMD.height / 2));
	}

	userPhotoBMD.copy(userVideoProcessedSprite, left, top, right - left - 1, bottom - top - 1, 0, 0, undefined, undefined, undefined, undefined, undefined, 1, 1, undefined, null, true);
	userPhotoBMD.update();

	userPhotoSprite = game.add.sprite(K_PROJECT_WIDTH / 2, K_PROJECT_HEIGHT / 2, userPhotoBMD);
...
makeMultiFrameEvent(2377, 2378, () => {
	for (let i = 0; i < userPhotoSprite.height; i += 10) {
		for (let j = 0; j < userPhotoSprite.width; j += 10) {
			const p = userPhotoBMD.getPixel(Math.round(j / userPhotoScale), Math.round(i / userPhotoScale));
			...
			const pVal = game.add.text(x, y, numeral, {font: "Arimo, sans-serif", fontSize: "8px", fontWeight: "bold", fill: color});
	...
	const userPhotoAlphaTween = game.add.tween(userPhotoSprite).to({alpha: 0}, 3000, Phaser.Easing.Linear.None, true, 1500, 0, false);
});
...
makeMultiFrameEvent(2828, 2883, () => {
	...
	if (landmarks) {
		trainingImageCelebrityTitle.x = K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2);
...
makeMultiFrameEvent(3450, 3451, () => {
	...
	const rightUserVideoSlide = game.add.tween(altUserVideoGroup.position).to( {x: (K_PROJECT_WIDTH / 2) - (K_PROJECT_WIDTH / 4), y: 0}, 200, Phaser.Easing.Linear.None, true, 0, 0, false);

	if (gameframe % K_VIDEO_PROCESSING_FRAME_INTERVAL === 0 || frame === 3450) {
		const s = 1 / K_VIDEO_PROCESSING_DOWNRES_FACTOR * 1.50;
		userVideoProcessedBMD.copy(userVideoSprite, 0, 0, Math.floor(userVideo.width), Math.floor(userVideo.height), 0, 0, undefined, undefined, undefined, undefined, undefined, s, s, undefined, null, true);
		userVideoProcessedBMD.update();
	...
	if (landmarks) {
		for (let j = 0; j < K_NUMBER_OF_LANDMARKS; j += 1) {
			...
			landmarkText[j].position = {x: landmarks[j][0] + altUserVideoGroup.position.x + userVideoProcessedSpriteLeftMask.position.x, y: landmarks[j][1] + altUserVideoGroup.position.y + userVideoProcessedSpriteLeftMask.position.y};
		}
	} else {
		for (let j = 0; j < K_NUMBER_OF_LANDMARKS; j += 1) {
...
makeMultiFrameEvent(3732, 3733, () => {
	...
	const gradientSlide = game.add.tween(recordedFeatureGroup.position).to( {x: 0, y: 0}, 200, Phaser.Easing.Linear.None, true, 0, 0, false);
	...
	landmarkPlaybackTimer.loop(60, () => {
		if (viewerLandmarks.length > 0 ) {
			...
			recordingOfYourFaceTitle.text = `Your face vectors \nrecorded ${((frame - landmarksData.frame) / 30).toFixed(1)} seconds ago `;
			for (let j = 0; j < K_NUMBER_OF_LANDMARKS; j += 1) {
				recordedLandmarkText[j].visible = true;
				recordedLandmarkText[j].position = {x: landmarksData.landmarks[j][0] + userVideoGroup.position.x + userVideoSprite.position.x, y: landmarksData.landmarks[j][1] + userVideoGroup.position.y + userVideoSprite.position.y};
			}
...
makeMultiFrameEvent(3912, 3913, () => {
	const rightUserVideoSlide = game.add.tween(altUserVideoGroup.position).to( {x: K_PROJECT_WIDTH, y: 0}, 200, Phaser.Easing.Linear.None, true, 0, 0, false);
...
makeMultiFrameEvent(4053, 4054, () => {
	...
	const recordedFeatureGroupSlide = game.add.tween(recordedFeatureGroup.position).to( {x: -(K_PROJECT_WIDTH / 2), y: 0}, 200, Phaser.Easing.Linear.None, true, 0, 0, false);
...
makeMultiFrameEvent(4297, 4298, () => {
	...
	const gradientTween = game.add.tween(overlayGroup.position).to({x: 0, y: 0}, 300, Phaser.Easing.Linear.None, true, 0, 0, false);
	...
	mouthPlaybackTimer.loop(60, () => {
		if (faceDataInRange.length > 0 ) {
			if (recordedFeaturePlaybackIndex > faceDataInRange.length - 1) {
				recordedFeaturePlaybackIndex = 0;
			}

			const landmarksData = faceDataInRange[recordedFeaturePlaybackIndex].landmarks;
			...
			if (landmarksData) {
				...
				if (e.sad) {
					mouthQuantificationTitle.text = `${(e.neutral).toFixed(6)} neutral\n${(e.happy).toFixed(6)} happy\n${(e.sad).toFixed(6)} sad\n${(e.angry).toFixed(6)} angry\n${(e.fearful).toFixed(6)} fearful\n${(e.disgusted).toFixed(6)} disgusted\n${(e.surprised).toFixed(6)} surprised`;
				}
			}
...
makeMultiFrameEvent(5246, 5247, () => {
	...
	if (landmarks) {
		noTitle.x = K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2);
...
makeMultiFrameEvent(5821, 5822, () => {
	...
	if (whiteNegative !== null && nonWhiteNegative !== null) {
		...
		finalRaceBias = preferWhite ? nonWhiteNegative - whiteNegative : whiteNegative - nonWhiteNegative;
		raceStatsBias.text = preferWhite ? `+${(finalRaceBias).toFixed(6)} \nwhite bias ` : `+${(finalRaceBias).toFixed(6)} \nBlack bias `;
...
makeMultiFrameEvent(6137, 6138, () => {
	...
	if (whiteNegative !== null && nonWhiteNegative !== null) {
		...
		const normalizedBlackPreference = (whiteNegative - nonWhiteNegative + 1.0) / 2.0;

		const standardDeviationsFromMean = (normalizedBlackPreference - meanGlobalBlackPreference) / standardDeviation;
		...
		// We only emit a visualization event if we got some signal from the user
		if (K_INSTALLATION_MODE) {
			socket.emit("datingPreference", [whitePct.toFixed(1), nonWhitePct.toFixed(1)]);
		}
	}

	higherOrderQuantificationTitle.visible = true;
	higherOrderQuantificationTitle.position = {x: K_PROJECT_WIDTH / 2, y: K_PROJECT_HEIGHT / 2};

	higherOrderTextBoxLeft = makeTextBox(K_PROJECT_WIDTH / 4 - (K_TEXT_BOX_SIZE / 2), K_PROJECT_HEIGHT / 2 - (K_TEXT_BOX_SIZE / 2), "White\nprofiles");
	higherOrderTextBoxRight = makeTextBox(K_PROJECT_WIDTH * 0.75 - (K_TEXT_BOX_SIZE / 2), K_PROJECT_HEIGHT / 2 - (K_TEXT_BOX_SIZE / 2), "Non-white profiles");
...
makeMultiFrameEvent(6260, 6261, () => {
	...
	if (kanyePos !== null && kanyeNeg !== null) {
		...
		const standardDeviationsFromMean = (normalizedKanyePreference - meanGlobalKanyePreference) / standardDeviation;
		...
		// We only emit a visualization event if we got some signal from the user
		if (K_INSTALLATION_MODE) {
			socket.emit("socialNews", [politicalPct.toFixed(1), gossipPct.toFixed(1)]);
		}
	}
...
makeMultiFrameEvent(6391, 6392, () => {
	...
	if (dogPos !== null && kanyePos !== null && nonWhiteNegative !== null) {
		...
		const standardDeviationsFromMean = (normalizedRepublicanness - meanGlobalRepublicanness) / standardDeviation;
		...
		higherOrderQuantificationRightPercent.text = `${republicanPct.toFixed(1)}% `;

		higherOrderQuantificationLeftPercent.fill = democratPct > republicanPct ? "#33cc33" : "#ff0000";
		higherOrderQuantificationRightPercent.fill = democratPct > republicanPct ? "#ff0000" : "#33cc33";

		// We only emit a visualization event if we got some signal from the user
		if (K_INSTALLATION_MODE) {
			socket.emit("politicalOrientation", [democratPct.toFixed(1), republicanPct.toFixed(1)]);
		}
	}
...
makeMultiFrameEvent(6731, 6732, () => {
	...
	const userVideoSlideRight = game.add.tween(userVideoGroup.position).to( {x: -(K_PROJECT_WIDTH / 4), y: 0}, 200, Phaser.Easing.Linear.None, true, 0, 0, false);
	const mapSlideLeft = game.add.tween(mapGroup.position).to( {x: K_PROJECT_WIDTH / 2, y: 0}, 200, Phaser.Easing.Linear.None, true, 0, 0, false);
...
makeMultiFrameEvent(6732, 7222, () => {
	...
	if (landmarks) {
		...
		mapGfx.lineTo(landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2) + (size / 2), landmarks[33][1]);

		const copyScale = pixelatedUserFaceBMD.width / (size / userVideoSpriteScale); 

		// TODO: Should this run at a different frequency using the video update interval, like our other video processing effects?
		pixelatedUserFaceBMD.copy(userVideoSprite, (landmarks[33][0] - (size / 2)) / userVideoSpriteScale, (landmarks[33][1] - (size / 2)) / userVideoSpriteScale, size / userVideoSpriteScale, size / userVideoSpriteScale, 0, 0, undefined, undefined, 0, 0, 0, copyScale, copyScale, 1, null, true);
		...
		pixelatedUserFaceSprite.scale.setTo(pixelatedUserFaceSpriteScale, pixelatedUserFaceSpriteScale);
		pixelatedUserFaceSprite.position = {x: xPos, y: yPos};
		pixelatedUserFaceSprite.visible = true;

		if (emotions) {
			userFaceRawEmotionsText.text = `[${(emotions["neutral"]).toFixed(6)} ${(emotions["happy"]).toFixed(6)} ${(emotions["sad"]).toFixed(6)} ${(emotions["angry"]).toFixed(6)}\n${(emotions["fearful"]).toFixed(6)} ${(emotions["disgusted"]).toFixed(6)} ${(emotions["surprised"]).toFixed(6)}]`;
...
makeMultiFrameEvent(8712, 8713, () => {
	...
	const cursorTween = game.add.tween(mouseCursor).to( {x: 1230, y: 20}, 400, Phaser.Easing.Linear.None, true, 0, 0, false);

	cursorTween.onComplete.addOnce(() => {
		doubleClickSFX.play();
		
		if (landmarks) {
			disney.x = K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2);
...
makeMultiFrameEvent(8763, 8764, () => {
	doubleClickSFX.play();
	twentiethCenturyFox.position = {x: K_PROJECT_WIDTH / 2, y: K_PROJECT_HEIGHT / 2};
});

makeMultiFrameEvent(8764, 8836, () => {
	if (landmarks) {
		twentiethCenturyFox.x = K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2);
...
makeMultiFrameEvent(8836, 8837, () => {
	doubleClickSFX.play();
	kelloggsCornFlakes.position = {x: K_PROJECT_WIDTH / 2, y: K_PROJECT_HEIGHT / 2};
});

makeMultiFrameEvent(8837, 8917, () => {
	if (landmarks) {
		kelloggsCornFlakes.x = K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2);
...
makeMultiFrameEvent(8917, 8918, () => {
	doubleClickSFX.play();
	yourLocalPizzeria.position = {x: K_PROJECT_WIDTH / 2, y: K_PROJECT_HEIGHT / 2};
});

makeMultiFrameEvent(8918, 8981, () => {
	if (landmarks) {
		yourLocalPizzeria.x = K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2);
...
makeMultiFrameEvent(8982, 9107, () => {
	if (landmarks) {
		forYourNextJob.x = K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2);
...
makeMultiFrameEvent(9151, 9152, () => {
	...
	if (dogPos !== null && menPos !== null && womenPos !== null && 
		whiteNegative !== null && nonWhiteNegative !== null && kanyePos !== null) {
		...
		const normalizedStupidity = (dogPos + Math.abs(menPos - womenPos) + Math.abs(whiteNegative - nonWhiteNegative) + kanyePos) / 4;

		const standardDeviationsFromMean = -(normalizedStupidity - meanGlobalStupidity) / standardDeviation;
...
makeMultiFrameEvent(9702, 9703, () => {
	function mentalHealthQuantification() {
		function selfLoathingQuantification() {
			...
			positiveSamples.push(getAveragePositiveEmotions(viewerFaceData, 585, 1025)); // dog AR filter bit
			negativeSamples.push(getAverageNegativeEmotions(viewerFaceData, 585, 1025));

			positiveSamples.push(getAveragePositiveEmotions(viewerFaceData, 2125, 2278)); // AI vision bit
			negativeSamples.push(getAverageNegativeEmotions(viewerFaceData, 2125, 2278));

			positiveSamples.push(getAveragePositiveEmotions(viewerFaceData, 3363, 3921)); // tophat/monocle AR filter + enumerated landmarks bit
			negativeSamples.push(getAverageNegativeEmotions(viewerFaceData, 3363, 3921));

			positiveSamples.push(getAveragePositiveEmotions(viewerFaceData, 6731, 7222)); // Geolocation correlation bit
			negativeSamples.push(getAverageNegativeEmotions(viewerFaceData, 6731, 7222));
			...
			if (positiveSamples.some(notNull) && negativeSamples.some(notNull)) {
				...
				// We only emit a visualization event if we got some signal from the user
				if (K_INSTALLATION_MODE) {
					socket.emit("selfImage", [isSelfLoathing, Math.abs(meanPositive - meanNegative).toFixed(6)]);
				}
			...
			reactionToYourselfLabel.text = "reaction to yourself: " + (isSelfLoathing ? "bad" : "good");
			reactionToYourselfLabel.position.x = userVideoGroup.x + userVideoSprite.x + userVideoSprite.width - 37;
		...
		for (let i = 0; i < viewerFaceData.length; i += 1) {
			if (viewerFaceData[i].sad) {
				const frameMean = (viewerFaceData[i].happy + viewerFaceData[i].sad + viewerFaceData[i].angry + 
					viewerFaceData[i].fearful + viewerFaceData[i].disgusted + viewerFaceData[i].surprised) / 6;
		...
		facialAffectLabel.text = "avg facial affect: " + (isMentallyIll ? "low" : "high");
		facialAffectLabel.position.x = userVideoGroup.x + userVideoSprite.x + userVideoSprite.width - 37;
	...
	if (dogPos !== null && iq !== null && republicanPct !== null) {
		...
		const standardDeviationsFromMean = (normalizedWealth - meanGlobalWealth) / standardDeviation;
	...
	estimatedIncomeLabel.position.x = userVideoGroup.x + userVideoSprite.x + userVideoSprite.width - 37;
...
makeMultiFrameEvent(9703, 10176, () => {
	...
	if (K_SHOW_BASEBALL_CARD && !baseballCardPicCaptured && landmarks && readyForBaseballCardCaptureFrame !== null && frame - readyForBaseballCardCaptureFrame > 5) {
		...
		baseballCard.copy(game.canvas, ((K_PROJECT_WIDTH - (userVideo.width * userVideoScale)) / 2), 0, userVideo.width * userVideoScale, userVideo.height * userVideoScale, 0, 0, undefined, undefined, undefined, undefined, undefined, 1, 1);
		
		let genderBiasText = "none";
		if (menPos !== null && womenPos !== null) {
			genderBiasText = preferMen ? `+${(menPos - womenPos).toFixed(6)} male` : `+${(womenPos - menPos).toFixed(6)} female`;
		}

		let raceBiasText = "none";
		if (whiteNegative !== null && nonWhiteNegative !== null) {
			const finalRaceBias = preferWhite ? nonWhiteNegative - whiteNegative : whiteNegative - nonWhiteNegative;
			raceBiasText = preferWhite ? `+${(finalRaceBias).toFixed(6)} white` : `+${(finalRaceBias).toFixed(6)} Black`;
		}

		const stoleMyFeelings = game.make.text(0, 0, "\"A.I. stole my feelings at \nwww.stealingurfeelin.gs\" ", {font: "Arimo, sans-serif", strokeThickness: 5, fontStyle: "italic", fontSize: "50px", fontWeight: "bold", fill: "#00ffff", align: "left"});
		...
		baseballCard.text(`likes dogs: ${likeDogs ? "yes" : "no"}`, 37, basex + 57, "bold 30px Arimo", "rgb(255, 255, 0)", true);
		baseballCard.text(`likes kanye west: ${likeKanye ? "yes" : "no"}`, 37, basex + 97, "bold 30px Arimo", "rgb(255, 255, 0)", true);
		baseballCard.text(`wants pizza: ${likePizza ? "yes" : "no"}`, 37, basex + 137, "bold 30px Arimo", "rgb(255, 255, 0)", true);
		baseballCard.text(`gender bias: ${genderBiasText}`, 37, basex + 177, "bold 30px Arimo", "rgb(255, 255, 0)", true);
		baseballCard.text(`racial bias: ${raceBiasText}`, 37, basex + 217, "bold 30px Arimo", "rgb(255, 255, 0)", true);
		baseballCard.text(`estimated IQ: ${iq}`, 37, basex + 257, "bold 30px Arimo", "rgb(255, 255, 0)", true);

		baseballCard.draw("sufStamp", baseballCard.width - 200 - 37, baseballCard.height - 100 - (K_BASEBALL_CARD_TEXT_FIELD_HEIGHT / 2), 200, 200);
...
makeMultiFrameEvent(10176, 10177, () => {
	...
	const fadeHat = game.add.tween(poorHat).to({alpha: 0.0}, 1000, Phaser.Easing.Linear.None, true, 0, 0, false);
	const fadeCrown = game.add.tween(crown).to({alpha: 0.0}, 1000, Phaser.Easing.Linear.None, true, 0, 0, false);
	const fadeLeftHeart = game.add.tween(leftHeart).to({alpha: 0.0}, 1000, Phaser.Easing.Linear.None, true, 0, 0, false);
	const fadeRightHeart = game.add.tween(rightHeart).to({alpha: 0.0}, 1000, Phaser.Easing.Linear.None, true, 0, 0, false);
	const fadeLeftProhibited = game.add.tween(leftProhibited).to({alpha: 0.0}, 1000, Phaser.Easing.Linear.None, true, 0, 0, false);
	const fadeRightProhibited = game.add.tween(rightProhibited).to({alpha: 0.0}, 1000, Phaser.Easing.Linear.None, true, 0, 0, false);
	const fadeBubble = game.add.tween(isMentallyIll ? thoughtBubbleSad : thoughtBubbleHappy).to({alpha: 0.0}, 1000, Phaser.Easing.Linear.None, true, 0, 0, false);

	const fadeIncomeLabel = game.add.tween(estimatedIncomeLabel).to({alpha: 0.0}, 1000, Phaser.Easing.Linear.None, true, 0, 0, false);
	const fadeSelfLoathingLabel = game.add.tween(reactionToYourselfLabel).to({alpha: 0.0}, 1000, Phaser.Easing.Linear.None, true, 0, 0, false);
	const fadeFacialAffectLabel = game.add.tween(facialAffectLabel).to({alpha: 0.0}, 1000, Phaser.Easing.Linear.None, true, 0, 0, false);

	userVideoProcessedSprite.alpha = 0.0;
	userVideoProcessedSprite.visible = true;

	const fadeInGreyscale = game.add.tween(userVideoProcessedSprite).to({alpha: 1.0}, 1000, Phaser.Easing.Linear.None, true, 0, 0, false);

	for (let i = 0; i < K_NUMBER_OF_LANDMARKS; i += 1) {
		landmarkText[i].alpha = 0.0;
		landmarkText[i].visible = true;

		const fadeInLandmarkText = game.add.tween(landmarkText[i]).to({alpha: 1.0}, 1000, Phaser.Easing.Linear.None, true, 0, 0, false);
	}
	...
	if (landmarks) {
		for (let j = 0; j < K_NUMBER_OF_LANDMARKS; j += 1) {
			landmarkText[j].visible = true;
			landmarkText[j].position = {x: landmarks[j][0] + userVideoGroup.position.x + userVideoProcessedSprite.position.x, y: landmarks[j][1] + userVideoGroup.position.y + userVideoProcessedSprite.position.y};
		}
	} else {
		for (let j = 0; j < K_NUMBER_OF_LANDMARKS; j += 1) {
			landmarkText[j].visible = false;
...
makeMultiFrameEvent(10355, 10356, () => {
	...
	userVideoProcessedSpriteLeftMask = game.add.sprite(0, 0, altUserVideoProcessedBMD);
	userVideoProcessedSpriteLeftMask.alpha = 0.0;
	userVideoProcessedSpriteLeftMask.scale.setTo(K_VIDEO_PROCESSING_DOWNRES_FACTOR, K_VIDEO_PROCESSING_DOWNRES_FACTOR);
	userVideoProcessedSpriteLeftMask.position = {x: K_PROJECT_WIDTH / 2 - (userVideoSprite.width / 2), y: 0};
	...
	const fadeInBinarized = game.add.tween(userVideoProcessedSpriteLeftMask).to({alpha: 1.0}, 300, Phaser.Easing.Linear.None, true, 0, 0, false);

	if (gameframe % K_VIDEO_PROCESSING_FRAME_INTERVAL === 0 || frame === 2125) {
		const s = 1 / K_VIDEO_PROCESSING_DOWNRES_FACTOR * 1.50;
		altUserVideoProcessedBMD.copy(userVideoSprite, 0, 0, Math.floor(userVideo.width), Math.floor(userVideo.height), 0, 0, undefined, undefined, undefined, undefined, undefined, s, s, undefined, null, true);
...
makeMultiFrameEvent(10388, 10389, () => {
	...
	finalBoundingBlinkTimer.loop(100, () => {
		boundingBox.visible = boundingBox.visible ? false : true;
	}, this);
...
makeMultiFrameEvent(10441, 10442, () => {
	if (K_BETA) {
		...
		game.add.text(0, 0, msg, {font: "Arimo, sans-serif", fontSize: "14px", fill: "#000000", backgroundColor: "#ffff1a"});
	}
	...
	creditTitle.position = {x: creditSqueezeOffset + K_PROJECT_WIDTH / 2, y: K_PROJECT_HEIGHT / 2};

	if (landmarks) {
		creditTitle.position = {x: creditSqueezeOffset + K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2), y: landmarks[33][1]};
	}
...
makeMultiFrameEvent(10625, 10626, () => {
	...
	if (K_SHOW_BASEBALL_CARD && baseballCardPicCaptured) {
		...
		creditTitle.setStyle({font: "Arial, Helvetica, sans-serif", fontSize: "24px", fontWeight: "bold", fill: "#FFFFFF"}, true);
		...
		bcElement.width = Math.round(userVideo.width * (K_PROJECT_HEIGHT / userVideo.height));
		bcElement.height = Math.round(userVideo.height * (K_PROJECT_HEIGHT / userVideo.height));
		...
		loader.onLoadComplete.addOnce(() => {
			...
			baseballCardSprite.position = {x: K_PROJECT_WIDTH / 4, y: K_PROJECT_HEIGHT / 2};

			baseballCardSprite.inputEnabled = true;
			baseballCardSprite.input.useHandCursor = true;

			const instructionsText = game.add.text(0, 0, "^^ click to download your personalized results ^^", {font: "Arimo, sans-serif", fontSize: "20px", fontWeight: "bold", fill: "#FFFFFF"});
			instructionsText.anchor.setTo(0.5, 0.5);
			instructionsText.position = {x: K_PROJECT_WIDTH / 4, y: K_PROJECT_HEIGHT / 2 + (baseballCardSprite.height / 2) + 24};

			const instructionsTextBlinkTimer = game.time.create(false);

			instructionsTextBlinkTimer.loop(800, () => {
...
makeMultiFrameEvent(11861, 11862, () => {
	creditTitle.text = "special thanks\nAMANDA CULKOWSKI\nJENNA XU\nNIC MONTANARO\nERIK LEE SNYDER\nALEC COIRO\nKURSTEN BRACCHI";
...
makeMultiFrameEvent(12086, 12087, () => {
	mozillaLogoWhite.position = {x: creditSqueezeOffset + K_PROJECT_WIDTH / 2, y: K_PROJECT_HEIGHT / 2};

	if (landmarks) {
		mozillaLogoWhite.position = {x: creditSqueezeOffset + K_PROJECT_WIDTH - landmarks[33][0] - ((K_PROJECT_WIDTH - userVideoSprite.width) / 2), y: landmarks[33][1]};
	}
