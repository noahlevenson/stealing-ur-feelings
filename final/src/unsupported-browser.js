const unsupportedBrowserState = {
  create: function () {
    ...
    const unsupportedText = game.add.text(
      0,
      0,
      `STEALING UR FEELINGS doesn't work with this browser. For this device, please use ${suggestedBrowsers}.`,
      {
    ...
    unsupportedText.position = {
      x: K_PROJECT_WIDTH / 2,
      y: K_PROJECT_HEIGHT / 2,
    };

    const unsupportedTextHeader = game.add.text(0, 0, "unsupported browser:", {
      font: "Arimo, sans-serif",
      fontSize: hsize,
      fontWeight: "bold",
      fill: "#eb4034",
    });
    unsupportedTextHeader.position = {
      x: unsupportedText.position.x - unsupportedText.width / 2,
      y:
        unsupportedText.y -
        unsupportedText.height / 2 -
        (fsize === "20px" ? 35 : 70),
    };
  },
};
