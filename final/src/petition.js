...
const petitionState = {
  create: function () {
    ...
    const smileToSignText = game.add.text(
      0,
      0,
      "Tell Snap to disclose if it is already using emotion recognition technology.",
      {
    ...
    smileToSignText.position = {
      x: K_PROJECT_WIDTH / 2,
      y: K_PROJECT_HEIGHT / 2 - 150,
    };

    const finePrintText = game.add.text(
      0,
      0,
      "*Data about your smile and face will not be stored nor used for any purpose other than launching ",
      {
        font: "Arimo, sans-serif",
        fontSize: "12px",
        fontWeight: "bold",
    ...
    finePrintText.position = {
      x: smileToSignText.x - smileToSignText.width / 2,
      y: K_PROJECT_HEIGHT - 100,
    };

    const thisWebsiteText = game.add.text(0, 0, "this website", {
      font: "Arimo, sans-serif",
      fontSize: "12px",
      fontWeight: "bold",
      fill: "#e6e6e6",
    ...
    thisWebsiteText.position = {
      x: finePrintText.x + finePrintText.width,
      y: finePrintText.y,
    };
    ...
    thisWebsiteText.events.onInputDown.add(() => {
      window.location.href =
        "https://foundation.mozilla.org/en/campaigns/snapchat-stop-stealing-our-feelings/";
    });

    const smileToSignSubText = game.add.text(
      0,
      0,
      "To read and sign Mozilla's petition, just ",
      {
        font: "Arimo, sans-serif",
        fontSize: "27px",
        fontWeight: "bold",
    ...
    smileToSignSubText.position = {
      x: smileToSignText.x - smileToSignText.width / 2,
      y: smileToSignText.y + smileToSignText.height / 2 + 16,
    };

    const smileBlinkingText = game.add.text(0, 0, "smile ", {
      font: "Arimo, sans-serif",
      fontSize: "27px",
      fontWeight: "bold",
      fill: "#ffff1a",
    ...
    smileBlinkingText.position = {
      x: smileToSignSubText.x + smileToSignSubText.width,
      y: smileToSignSubText.y,
    };

    const petitionBlinkTimer = game.time.create(false);

    petitionBlinkTimer.loop(
      400,
      () => {
    ...
    redirectingText = game.add.text(0, 0, "Redirecting...", {
      font: "Arimo, sans-serif",
      fontSize: "20px",
      fontWeight: "bold",
      fill: "#ffffff",
    ...
    redirectingText.position = {
      x: K_PROJECT_WIDTH / 2,
      y: K_PROJECT_HEIGHT / 2 + 160,
    };
    redirectingText.visible = false;

    noPetitionFaceDetectedText = game.add.text(
      0,
      0,
    ...
    noPetitionFaceDetectedText.position = {
      x: K_PROJECT_WIDTH / 2,
      y: K_PROJECT_HEIGHT / 2 + 20,
    };
    noPetitionFaceDetectedText.visible = false;

    const altPetitionLinkText = game.add.text(0, 0, "(or click here)", {
      font: "Arimo, sans-serif",
      fontSize: "27px",
      fontWeight: "bold",
      fill: "#ffff1a",
    ...
    altPetitionLinkText.position = {
      x: smileBlinkingText.x + smileBlinkingText.width,
      y: smileToSignSubText.y,
    };
    ...
    userFailedToSmileTimeout = setTimeout(() => {
      ...
      const altLinkTween = game.add
        .tween(altPetitionLinkText)
        .to({ alpha: 1.0 }, 2000, Phaser.Easing.Linear.None, true, 0, 0, false);
