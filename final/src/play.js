...
function handlePlayDetections(detections, cachedFrame) {
	if (detections) {
		...
		if (detections.expressions && getLandmarks) {
			handleViewerFaceData(cachedFrame, viewerFaceData, landmarks, detections.expressions); 
			emotions = detections.expressions;
		} else if (detections.expressions && !getLandmarks) {
			handleViewerFaceData(cachedFrame, viewerFaceData, null, detections.expressions);
			emotions = detections.expressions;
		} else if (!detections.expressions && getLandmarks) {
		...
		if (K_INSTALLATION_MODE) {
			const faceData = new viewerFaceDataStruct(cachedFrame, landmarks, detections.expressions || null);
...
function drawBigMouthModel(l) {
	...
	mouthModel.moveTo(landmarks[K_FACE_MOUTH_OUTER[0]][0], landmarks[K_FACE_MOUTH_OUTER[0]][1]);

	for (let i = 1; i < K_FACE_MOUTH_OUTER.length; i += 1) {
		mouthModel.lineTo(landmarks[K_FACE_MOUTH_OUTER[i]][0], landmarks[K_FACE_MOUTH_OUTER[i]][1]);
	}	

	mouthModel.moveTo(landmarks[K_FACE_MOUTH_INNER_TOP[0]][0], landmarks[K_FACE_MOUTH_INNER_TOP[0]][1]);

	for (let i = 1; i < K_FACE_MOUTH_INNER_TOP.length; i += 1) {
		mouthModel.lineTo(landmarks[K_FACE_MOUTH_INNER_TOP[i]][0], landmarks[K_FACE_MOUTH_INNER_TOP[i]][1]);
	}	

	mouthModel.moveTo(landmarks[K_FACE_MOUTH_INNER_BOTTOM[0]][0], landmarks[K_FACE_MOUTH_INNER_BOTTOM[0]][1]);

	for (let i = 1; i < K_FACE_MOUTH_INNER_BOTTOM.length; i += 1) {
		mouthModel.lineTo(landmarks[K_FACE_MOUTH_INNER_BOTTOM[i]][0], landmarks[K_FACE_MOUTH_INNER_BOTTOM[i]][1]);
	}	
}
...
const playState = {
	create: function() {
		...
		if (userVideoBackplateBMD) {
			...
			userVideoBackplateSprite.scale.setTo(userVideoSpriteScale, userVideoSpriteScale);
		...
		userVideoProcessedBMD = game.add.bitmapData(Math.round(userVideoSprite.width / K_VIDEO_PROCESSING_DOWNRES_FACTOR), Math.round(userVideoSprite.height / K_VIDEO_PROCESSING_DOWNRES_FACTOR));
		...
		angryLabelText = game.add.text(K_PROJECT_WIDTH / 4, 0, "angry", {font: "Arimo, sans-serif", fontSize: "24px", fill: "#000000"});
		angryLabelText.anchor.setTo(0.5, 0.5);
		angryLabelText.visible = false;
		sadLabelText = game.add.text(K_PROJECT_WIDTH / 4, 0, "sad", {font: "Arimo, sans-serif", fontSize: "24px", fill: "#000000"});
		sadLabelText.anchor.setTo(0.5, 0.5);
		sadLabelText.visible = false;
		surprisedLabelText = game.add.text(K_PROJECT_WIDTH / 4, 0, "surprised", {font: "Arimo, sans-serif", fontSize: "24px", fill: "#000000"});
		surprisedLabelText.anchor.setTo(0.5, 0.5);
		surprisedLabelText.visible = false;
		happyLabelText = game.add.text(K_PROJECT_WIDTH / 4, 0, "happy", {font: "Arimo, sans-serif", fontSize: "24px", fill: "#000000"});
		...
		userVideoStatusText = game.add.text(K_PROJECT_WIDTH / 4 + 20, 20, "face camera and check lighting ", {font: "Arimo, sans-serif", strokeThickness: 5, fontStyle: "italic", fontSize: "30px", fontWeight: "bold", fill: "#ff0066"});
		...
		userVideoMaskQuarterLeft.drawRect(K_PROJECT_WIDTH / 4, 0, K_PROJECT_WIDTH, K_PROJECT_HEIGHT);
		userVideoMaskQuarterLeft.visible = false;

		userVideoProcessedSpriteLeftMask = game.add.sprite(0, 0, userVideoProcessedBMD);
		...
		for (let i = 0; i < K_NUMBER_OF_LANDMARKS; i += 1) {
			landmarkText[i] = game.add.text(0, 0, i, {font: "Arimo, sans-serif", fontSize: "7px", fill: "#80ff00"});
			...
			playButton.events.onInputDown.addOnce(() => {
				...
				if (K_MULTITHREADING) {
					createImageBitmap(userVideo.video).then((img) => {
						cvWorker.postMessage({fakeWindow: fakeWindow, fakeDocument: fakeDocument, img: img, opcode: 1, frame: frame, getEmotions: getEmotions}, [img]);
						filmVideo.play();
						activated = true;
					});
				} else {
					...
					const detections = faceapi.detectSingleFace(clmCanvas.canvas, options).withFaceLandmarks(true).withFaceExpressions(); 
					detections.then(() => {
						filmVideo.play();
						activated = true;
					});
		...
		threeTitle = game.add.text(0, 0, "3", {font: "Arimo, sans-serif", fontSize: "675px", fontWeight: "bold", fill: "#ff3399"});
		...
		twoTitle = game.add.text(0, 0, "2", {font: "Arimo, sans-serif", fontSize: "675px", fontWeight: "bold", fill: "#4dff4d"});
		...
		oneTitle = game.add.text(0, 0, "1", {font: "Arimo, sans-serif", fontSize: "675px", fontWeight: "bold", fill: "#ffffff"});
		...
		techQuizTitle = game.add.text(0, 0, "TECH\nQUIZ", {font: "Arimo, sans-serif", fontSize: "340px", fontWeight: "bold", fill: "#ffff1a"});
		...
		dogStats = game.add.text(0, 0, "1.000 \n positive ", {font: "Arimo, sans-serif", fontSize: "60px", fontWeight: "bold", strokeThickness: 5, fontStyle: "italic", fill: "#33cc33"});
		...
		genderStatsBias = game.add.text(0, 0, "+1.000 \n female bias ", {font: "Arimo, sans-serif", fontSize: "60px", fontWeight: "bold", strokeThickness: 5, fontStyle: "italic", fill: "#33cc33"});
		...
		kanyeTitle = game.add.text(0, 0, "Your reaction to Kanye", {font: "Arimo, sans-serif", fontSize: "82px", fontWeight: "bold", fill: "#000000", backgroundColor: "#ffff1a"});
		kanyeTitle.anchor.setTo(0.5, 0.5);
		kanyeTitle.x = K_PROJECT_WIDTH + kanyeTitle.width / 2;
		kanyeTitle.y = 142;
		kanyeTitle.visible = true;

		kanyeBias = game.add.text(0, 0, "+1.000 positive", {font: "Arimo, sans-serif", fontSize: "112px", fontWeight: "bold", fill: "#33cc33", backgroundColor: "#000000"});
		...
		pizzaTitle = game.add.text(0, 0, "Your reaction to pizza", {font: "Arimo, sans-serif", fontSize: "82px", fontWeight: "bold", fill:"#000000", backgroundColor: "#ffff1a"});
		pizzaTitle.anchor.setTo(0.5, 0.5);
		pizzaTitle.x = K_PROJECT_WIDTH + pizzaTitle.width / 2;
		pizzaTitle.y = 142;
		pizzaTitle.visible = true;

		pizzaBias = game.add.text(0, 0, "1.000 positive", {font: "Arimo, sans-serif", fontSize: "112px", fontWeight: "bold", fill: "#33cc33", backgroundColor: "#000000"});
		...
		standbyForAIVisionTitle = game.add.text(0, 0, "stand by for \nA.I. vision ", {font: "Arimo, sans-serif", fontSize: "150px", fontWeight: "bold", strokeThickness: 5, fontStyle: "italic", fill: "#ffffff"});
		...
		actualImagesUsedToTrainTitle = game.add.text(0, 0, "*actual images \nthat trained \nour A.I. ", {font: "Arimo, sans-serif", strokeThickness: 5, fontStyle: "italic", fontSize: "70px", fontWeight: "bold", fill: "#ffffff"});
		...
		instructionsTitle = game.add.text(0, 0, "smile!!!", {font: "Arimo, sans-serif", fontSize: "150px", fontWeight: "bold", fill: "#ffff1a", backgroundColor: "#00000"});
		...
		raceStatsBias = game.add.text(0, 0, "+1.000 \nBlack bias ", {font: "Arimo, sans-serif", fontSize: "60px", fontWeight: "bold", strokeThickness: 5, fontStyle: "italic", fill: "#33cc33"});
		...
		for (let i = 0; i < K_NUMBER_OF_LANDMARKS; i += 1) {
			recordedLandmarkText[i] = game.add.text(0, 0, i, {font: "Arimo, sans-serif", fontSize: "8px", fontWeight: "bold", strokeThickness: 5, fill: "#ff99ff"});
		...
		recordingOfYourFaceTitle = game.add.text(0, 0, "your face vectors \nrecorded 0 seconds ago ", {font: "Arimo, sans-serif", fontSize: "30px", strokeThickness: 5, fontStyle: "italic", fontWeight: "bold", fill: "#00ffff", backgroundColor: "#ffff1a"});
		...
		const pictureFrameScale = (K_PROJECT_HEIGHT - 100) / pictureFrameSprite.height;
		...
		museumCardSprite = game.add.sprite(pictureFrameSprite.x, pictureFrameSprite.y + pictureFrameSprite.height * 0.20, "museumCard");
		...
		yourMouthShapeTitle = game.add.text(0, 0, "YOUR MOUTH\nMOMENTS AGO", {font: "Arimo, sans-serif", fontSize: "70px", fontWeight: "bold", fill: "#ffff1a", backgroundColor: "#00000"});
		...
		mouthQuantificationTitle = game.add.text(0, 0, "0.000000 neutral\n0.000000 happy\n0.000000 sad\n0.000000 angry\n0.000000 fearful\n0.000000 disgusted\n0.000000 surprised", {font: "Arimo, sans-serif", fontSize: "28px", fontWeight: "bold", fill: "#ffffff", backgroundColor: "#00000"});
		...
		noTitle = game.add.text(0, 0, "NO!!!", {font: "Arimo, sans-serif", fontSize: "420px", fontWeight: "bold", fill: "#ff0000"});
		...
		higherOrderQuantificationTitle = game.add.text(0, 0, "you \nwill \nsee ", {font: "Arimo, sans-serif", fontSize: "66px", fontWeight: "bold", strokeThickness: 5, fontStyle: "italic", fill: "#00ffff", backgroundColor: "#ff33ff"});
		...
		higherOrderQuantificationLeftPercent = game.add.text(0, 0, "50% ", {font: "Arimo, sans-serif", fontSize: "96px", fontWeight: "bold", strokeThickness: 5, fontStyle: "italic", fill: "#33cc33"});
		...
		higherOrderQuantificationRightPercent = game.add.text(0, 0, "50% ", {font: "Arimo, sans-serif", fontSize: "96px", fontWeight: "bold", strokeThickness: 5, fontStyle: "italic", fill: "#ff0000"});
		...
		mapEmotionLabel = game.add.text(0, 0, "feels happy ", {font: "Arimo, sans-serif", fontSize: "72px", fontWeight: "bold", fill: "#0099ff", strokeThickness: 5, fontStyle: "italic"});
		...
		userFaceRawEmotionsText  = game.add.text(0, 0, "[.000000 .000000 .000000 .000000\n.000000 .000000 .000000]", {font: "Arimo, sans-serif", fontSize: "18px", fontWeight: "bold", fill: "#000000"});
		userFaceRawEmotionsText.visible = false;

		estimatedIQTitle = game.add.text(0, 0, "Your estimated IQ ", {font: "Arimo, sans-serif", fontSize: "72px", fontWeight: "bold", strokeThickness: 5, fontStyle: "italic", fill: "#00ffff", backgroundColor: "#ffff1a"});
		...
		iqStatTitle = game.add.text(0, 0, "100 ", {font: "Arimo, sans-serif", fontSize: "440px", fontWeight: "bold", fill: "#33cc33", strokeThickness: 5, fontStyle: "italic"});
		...
		estimatedIncomeLabel = game.add.text(0, 0, "estimated income: $" + estimatedIncome, {font: "Arimo, sans-serif", fontSize: "36px", fontWeight: "bold", fill: "#000000", backgroundColor: "#ffff1a"});
		estimatedIncomeLabel.anchor.setTo(1.0, 1.0);
		estimatedIncomeLabel.visible = false;

		facialAffectLabel = game.add.text(1000, 1000, "avg facial affect: " + (isMentallyIll ? "low" : "high"), {font: "Arimo, sans-serif", fontSize: "36px", fontWeight: "bold", fill: "#000000", backgroundColor: "#ffff1a"});
		facialAffectLabel.anchor.setTo(1.0, 1.0);
		facialAffectLabel.visible = false;

		reactionToYourselfLabel = game.add.text(1000, 1000, "reaction to yourself: " + (isSelfLoathing ? "bad" : "good"), {font: "Arimo, sans-serif", fontSize: "36px", fontWeight: "bold", fill: "#000000", backgroundColor: "#ffff1a"});
		reactionToYourselfLabel.anchor.setTo(1.0, 1.0);
		reactionToYourselfLabel.visible = false;

		creditTitle = game.add.text(0, 0, "written, directed and programmed by\nNOAH LEVENSON", {font: "Arimo, sans-serif", fontSize: "42px", fontWeight: "bold", fill: "#FFFFFF"});
		...
		if (K_SHOW_BASEBALL_CARD) {
			...
			baseballCard = game.add.bitmapData(Math.round(userVideo.width * userVideoScale), Math.round(userVideo.height * userVideoScale) + K_BASEBALL_CARD_TEXT_FIELD_HEIGHT);
		}
		...
		snapchatEmojiEmitter = game.add.emitter(game, 0, 0, K_EMOJI_EXPLOSION_PARTICLES);
		snapchatEmojiEmitter.x = 0;
		snapchatEmojiEmitter.y = -200;
		snapchatEmojiEmitter.bounce.setTo(0.5, 0.5);

		snapchatEmojiEmitter.makeParticles("secretEmoji", 0, K_EMOJI_EXPLOSION_PARTICLES, true, true);

		applauseEmojiEmitter = game.add.emitter(game, 0, 0, K_EMOJI_EXPLOSION_PARTICLES);
		applauseEmojiEmitter.x = 0;
		applauseEmojiEmitter.y = -200;
		applauseEmojiEmitter.bounce.setTo(0.5, 0.5);

		applauseEmojiEmitter.makeParticles("moneyFaceEmoji", 0, K_EMOJI_EXPLOSION_PARTICLES, true, true);
		...
		if (activated) {
			...
			if (frame % K_CV_REFRESH_INTERVAL === 0) {
				if (K_MULTITHREADING) {
					...
					if (!getEmotions) {
						if (K_RUN_CV_ASYNC) {
							detections = faceapi.detectSingleFace(clmCanvas.canvas, options).withFaceLandmarks(true); 
						} else {
							detections = await faceapi.detectSingleFace(clmCanvas.canvas, options).withFaceLandmarks(true); 
						}
