function drawSunglasses() {
	...
	const pline = new Phaser.Line(leftPupil[0] + userVideoSprite.position.x, leftPupil[1] + userVideoSprite.position.y, rightPupil[0] + userVideoSprite.position.x, rightPupil[1] + userVideoSprite.position.y);
	...
	if (detections) {
		...
		if (detections.expressions) {
			if (validationStateFaceData.length < 200000) {
				handleViewerFaceData(validateFrame, validationStateFaceData, landmarks, detections.expressions);
			} 
			
			emotions = detections.expressions
		} else {
			if (validationStateFaceData.length < 200000) {
				handleViewerFaceData(validateFrame, validationStateFaceData, landmarks, null);
			} 
		}

		if (K_INSTALLATION_MODE) {
			const faceData = new viewerFaceDataStruct(validateFrame, landmarks, detections.expressions || null);
...
const validateState = {
	create: function() {
		...
		userVideoMaskQuarterRight.drawRect(0, 0, K_PROJECT_WIDTH / 2, K_PROJECT_HEIGHT)
		...
		const meetTheNewAI = game.add.text(0, 0, "\"meet the new A.I. \nthat knows you better \nthan you know yourself\" ", {font: "Arimo, sans-serif", strokeThickness: 5, fontStyle: "italic", fontSize: "30px", fontWeight: "bold", fill: "#00ffff", align: "right"});
		...
		const directedBy = game.add.text(0, 0, " written, directed and programmed \n by noah levenson  (6 min) ", {font: "Arimo, sans-serif", strokeThickness: 5, fontStyle: "italic", fontSize: "20px", fontWeight: "bold", fill: "#ffffff", align: "right"});
		directedBy.lineSpacing = -5;
		directedBy.setShadow(2, 2, "#000000");
		directedBy.anchor.setTo(1.0, 0.0);
		directedBy.position = {x: K_PROJECT_WIDTH - 20, y: 20 + meetTheNewAI.height};

		const tribecaLaurel = game.add.sprite(0, 0, "tribecaLaurel");
		tribecaLaurel.scale.setTo(0.5, 0.5);
		tribecaLaurel.anchor.setTo(1.0, 0.0);
		tribecaLaurel.position = {x: K_PROJECT_WIDTH - 20, y: directedBy.y + directedBy.height + 5};

		const ciffLaurel = game.add.sprite(0, 0, "ciffLaurel");
		ciffLaurel.scale.setTo(0.5, 0.5);
		ciffLaurel.anchor.setTo(1.0, 0.0);
		ciffLaurel.position = {x: K_PROJECT_WIDTH - 20, y: tribecaLaurel.y + tribecaLaurel.height + 5};   

		const ridmLaurel = game.add.sprite(0, 0, "ridmLaurel");
		ridmLaurel.scale.setTo(0.5, 0.5);
		ridmLaurel.anchor.setTo(1.0, 0.0);
		ridmLaurel.position = {x: K_PROJECT_WIDTH - 20, y: ciffLaurel.y + ciffLaurel.height + 5};
		...
		opencityLaurel.position = {x: K_PROJECT_WIDTH - 20, y: ridmLaurel.y + ridmLaurel.height + 5};
		...
		pressRedButtonToStart = game.add.text(0, 0, "click smiley to start ", {font: "Arimo, sans-serif", strokeThickness: 5, fontStyle: "italic", fontSize: "40px", fontWeight: "bold", fill: "#ffff1a", align: "right"});
		pressRedButtonToStart.setShadow(2, 2, "#000000");   
		pressRedButtonToStart.anchor.setTo(0.5, 0.5);
		pressRedButtonToStart.position = {x: K_PROJECT_WIDTH / 2, y: K_PROJECT_HEIGHT - 75};
		pressRedButtonToStart.visible = false;
		...
		faceCameraAndCheckLighting = game.add.text(0, 0, "face camera and check lighting ", {font: "Arimo, sans-serif", strokeThickness: 5, fontStyle: "italic", fontSize: "40px", fontWeight: "bold", fill: "#ff0066", align: "right"});
		faceCameraAndCheckLighting.setShadow(2, 2, "#000000");
		faceCameraAndCheckLighting.anchor.setTo(0.5, 0.5);
		faceCameraAndCheckLighting.position = {x: K_PROJECT_WIDTH / 2, y: K_PROJECT_HEIGHT - 75};
		faceCameraAndCheckLighting.visible = true;

		notValidatedTimer = game.time.create(false);

		notValidatedTimer.loop(600, () => {
			faceCameraAndCheckLighting.text = faceCameraAndCheckLighting.text === "" ? "face camera and check lighting " : "";
		});
		...
		validatedTimer.loop(200, () => {
			pressRedButtonToStart.text = pressRedButtonToStart.text === "" ? "click smiley to start " : "";
		});

		validatedTimer.start();

		if (K_MOBILE) {
			mobileTapText = game.add.text(0, 0, "tap", {font: "Arimo, sans-serif", fontSize: "44px", fill: "rgba(255, 255, 255, 0.5)"});
			mobileTapText.anchor.setTo(0.5, 0.5);
			mobileTapText.position = {x: K_PROJECT_WIDTH * 0.75, y: K_PROJECT_HEIGHT / 2};
		...
		if (validateFrame % K_CV_REFRESH_INTERVAL === 0) {
			if (K_MULTITHREADING) {
				createImageBitmap(userVideo.video).then((img) => {
					cvWorker.postMessage({fakeWindow: fakeWindow, fakeDocument: fakeDocument, img: img, opcode: 1, getEmotions: true, frame: validateFrame}, [img]);
				});
			} else {
				updateCLM(clmCanvas);
